...
def solve_dsa(p, agents, iteration_number=0):  # Distributed stochastic algorithm
    cost_per_iteration = np.array([0] * 1000)
    ...
    while iteration_number < 1000:  # termination condition
...
def graph_operator(p1, dsa1_results=np.array([0] * 1000), dsa2_results=np.array([0] * 1000),
                   mgm_results=np.array([0] * 1000), at_least_10_runs=10):  # solve by 2 different algorithms (dsa/mgm)
